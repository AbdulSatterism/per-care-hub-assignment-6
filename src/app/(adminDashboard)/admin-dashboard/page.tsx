...
const AdminHome = () => {
  ...
  const totalPosts = animalData?.data?.length || 0;
  const totalLikes = animalData?.data?.reduce(
    (acc: any, post: { like: any }) => acc + (post.like || 0),
    0
  );
  const totalDislikes = animalData?.data?.reduce(
    (acc: any, post: { disLike: any }) => acc + (post.disLike || 0),
    0
  );
  const totalFollowers = animalData?.data?.reduce(
